	...
	useEffect(() => {
		...
		setMessages((prev) => {
			...
			return [
				...
				{
					role: "system",
					content: `Skor pengguna: ${scorePct}% (${correct}/${total}).\n` +
  ...
  idx?: number;
	...
	return (
		<div className={styles.innerWrapperMini}>
		  ...
		  {item && item.property?.includes("image-question") && (
			<img className={styles.imageQuiz} srcSet={item?.image} />
		  )}

		  {item && item.property?.includes("table") && (
		  ...
		  <div className={styles.resultAnswer}>
			...
			<RadioGroup disabled={true} defaultValue={answer}>
			  {item && Object.entries(item.options).map((itx) => {
				return (
				  <div
					key={itx[0]}
					className={
				  ...
				  >
					...
					<Label htmlFor={"id-" + itx[0]}>
					  {item && item.property?.includes("image-answer") ? (
...
export default function TestResultPage() {
	...
	useEffect(() => {
		async function fetchTest() {
			try {
			  	const dataAttempt = JSON.parse(localStorage.getItem("testResult"));
				const resLoad = await axim(`/api/load-test`).get()
				const dataLoad = await resLoad.json() as { content: { questions: Question[] } } 

			 	setTest(dataLoad.content.questions);
			  	setTestTitle("Test");
			  	setResults(dataAttempt || []);
	...
	<div className={styles.resultRoot}>
	  ...
	  <div className={styles.contentWrapper}>
		...
		<div className={styles.innerWrapper}>
		  <h1 style={{ textAlign: "center" }}>Test Selesai!</h1>
